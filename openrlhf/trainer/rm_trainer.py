...
class RewardModelTrainer(ABC):
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            for chosen_ids, c_mask, reject_ids, r_mask, margin, meta_infos in self.train_dataloader:
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}):
        ...
        if global_step % args.save_steps == 0:
            ...
            self.strategy.save_model(self.model, self.tokenizer, os.path.join(args.ckpt_path, tag))
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            rewards = []
            loss_sum = 0
            for chosen_ids, c_mask, reject_ids, r_mask, margin, meta_infos in eval_dataloader:
                chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                reject_ids = reject_ids.squeeze(1).to(torch.cuda.current_device())
                r_mask = r_mask.squeeze(1).to(torch.cuda.current_device())
    ...
    def test(self, eval_dataloader, get_token_info = False, output_file=None):
        ...
        all_info_dict_mode = {'c_router_logits': [], 'r_router_logits': [], 'chosen_ids': [], 'reject_ids':[], 'c_mask':[], 'r_mask':[], 'loss':[], 'acc':[], 'tags':[]}
        ...
        with torch.no_grad():
            ...
            for chosen_ids, c_mask, reject_ids, r_mask, margin, meta_infos in eval_dataloader:
                ...
                chosen_reward, reject_reward, _, output = self.concatenated_forward(
                    self.model, chosen_ids, c_mask, reject_ids, r_mask, return_all = True
                )
                loss = self.loss_fn(chosen_reward, reject_reward, margin, return_all = True)

                rewards += [chosen_reward.flatten(), reject_reward.flatten()]
                acc_bool = chosen_reward > reject_reward
                acc_list.extend(acc_bool.float().flatten().tolist())
                tags.extend([meta["tag"] for meta in meta_infos])
                acc += acc_bool.float().mean().item()

                loss_sum += loss.mean().item()
                step_bar.update()

                if get_token_info:
                    all_info_dict['tags'] = ([meta["tag"] for meta in meta_infos])
                    all_info_dict['acc'] = (acc_bool.float().flatten().tolist())
                    all_router_logits = torch.stack([t.reshape(chosen_ids.shape[0] + reject_ids.shape[0], -1, 60) for t in output.router_logits], dim=2).detach().cpu().tolist()
                    all_info_dict['c_router_logits'] = (all_router_logits[:chosen_ids.shape[0]])
                    all_info_dict['r_router_logits'] = (all_router_logits[chosen_ids.shape[0]:])
                    all_info_dict['c_mask'] = (c_mask.detach().cpu().tolist())
                    all_info_dict['r_mask'] = (r_mask.detach().cpu().tolist())
                    all_info_dict['chosen_ids'] = (chosen_ids.detach().cpu().tolist())
                    all_info_dict['reject_ids'] = (reject_ids.detach().cpu().tolist())
            ...
            bar_dict = {
                ...
                f"acc_mean": acc_mean,
                f"reward_mean": reward_mean.item(),
                f"reward_std": reward_std.item(),
            }
            self.strategy.print(bar_dict)
            self.strategy.print("===========Detail ACC==========")
