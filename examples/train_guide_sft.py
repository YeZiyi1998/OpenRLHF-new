...
from openrlhf.trainer import GuideSFTTrainer
...
def train(args):
    ...
    def load_data(path, is_test=False, is_arrow=False):
        if is_arrow:
    ...
    train_data = load_data(args.dataset, is_test=False, is_arrow = True)
    if args.max_samples != -1:
        train_data = train_data.select(range(min(args.max_samples, len(train_data))))
    eval_data = load_data(args.eval_dataset, is_test=False, is_arrow = True)
    # test_data = 
    ...
    eval_dataloader = strategy.setup_dataloader(
        eval_dataset, args.micro_eval_batch_size, True, False, eval_dataset.collate_fn
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--micro_eval_batch_size", type=int, default=8)
    ...
    parser.add_argument("--ratio", default=1e-6, type=float)
