...
class PairWiseLoss(nn.Module,):
    ...
    def forward(
        self, chosen_reward: torch.Tensor, reject_reward: torch.Tensor, margin: torch.Tensor = None, return_all = False
    ) -> torch.Tensor:
...
class PenaltyRMLoss(nn.Module):
    """
    Pairwise Loss for Reward Model
    Details: https://arxiv.org/pdf/2403.17297
    """

    def forward(
        self, chosen_reward: torch.Tensor, reject_reward: torch.Tensor, margin: torch.Tensor = None
    ) -> torch.Tensor:
        sigmoid_output = (1 + torch.exp(reject_reward - chosen_reward))
        ranking_loss = -1 * ((1 - 2 * torch.max(0, sigmoid_output - 0.5)) ** 2) * torch.log(sigmoid_output)
        penalty_loss = -1 * (
                torch.log(5 - chosen_reward) + torch.log(chosen_reward + 5)
                + torch.log(5 - reject_reward) + torch.log(reject_reward + 5)
                             )

        loss = ranking_loss.mean() + 0.02 * penalty_loss.mean()
        return loss
