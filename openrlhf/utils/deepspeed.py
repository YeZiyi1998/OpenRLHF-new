...
class DeepspeedStrategy(ABC):
    ...
    def create_optimizer2(self, models, **kwargs) -> Optimizer:
        optim_params = []
        for model in models:
            if model is None:
                continue
    ...
    def get_ds_train_config(self, is_actor):
        # import json
        # ds_config1 = json.load(open('/eb_data/rlhf/yezy/OpenRLHF-new/run_scripts/ds_config.json'))
        # DS Config
        ...
        # for k in ds_config1.keys():
        #     if type(ds_config1[k]) is dict:
        #         for k2 in ds_config1[k].keys():
        #             if k2 not in ds_config1[k].keys():
        #                 ds_config[k][k2] = ds_config1[k][k2]
        #     elif k not in ds_config.keys():
        #         ds_config[k] = ds_config1[k]
